...
def fetch_posts_with_playwright(sec_uid: str, display_name: str) -> Tuple[Optional[List[Dict]], str]:
    ...
    def fetch_from_share_page(browser, host: str, tag: str, path_prefix: str = '/share/user/'):
        ...
        try:
            ...
            # 通过页面上下文fetch调用API，实现分页+不同aweme_type
            # 页面的JS会自动生成a_bogus签名，绕过反爬虫
            extra_params = ''
            try:
                # 先从页面提取web_id/device_id等必要参数
                page_params = page.evaluate("""() => {
                    const params = {};
                    // 从cookie或window对象提取
                    try {
                        const cookies = document.cookie;
                        const m = cookies.match(/webid=([^;]+)/);
                        if (m) params.web_id = m[1];
                    } catch(e) {}
                    // 从URL中提取已使用的参数
                    try {
                        const perfEntries = performance.getEntriesByType('resource');
                        for (const e of perfEntries) {
                            if (e.name.includes('aweme/post')) {
                                const url = new URL(e.name);
                                params.web_id = params.web_id || url.searchParams.get('web_id') || '';
                                params.device_id = url.searchParams.get('device_id') || '';
                                params.user_cip = url.searchParams.get('user_cip') || '';
                                break;
                            }
                        }
                    } catch(e) {}
                ...
                web_id = page_params.get('web_id', '')
                device_id = page_params.get('device_id', '')
                if web_id:
                    extra_params += f'&web_id={web_id}'
                if device_id:
                    extra_params += f'&device_id={device_id}'

                cursor = last_cursor[0] or 0
                for page_num in range(3):
                    if len(captured_awemes) >= max_posts:
                        break
                    api_url = f'/web/api/v2/aweme/post/?reflow_source=reflow_page&sec_uid={sec_uid}&count=20&max_cursor={cursor}{extra_params}'
                    result = page.evaluate("""async (url) => {
                        try {
                            const resp = await fetch(url, {credentials: 'include'});
                            const data = await resp.json();
                            const al = data.aweme_list || [];
                            return {
                                count: al.length,
                    ...
                    if not result or result.get('error'):
                        print(f"  [{tag}-page{page_num+1}] fetch失败: {result.get('error','?') if result else 'no result'}")
                        break
                    items = result.get('items', [])
                    if items:
                        cnt = add_awemes(items, f'{tag}-page{page_num+1}', trust_context=True)
                        print(f"  [{tag}-page{page_num+1}] cursor={cursor} 获取{len(items)}条 新增{cnt}条 has_more={result.get('has_more')}")
                    else:
                        print(f"  [{tag}-page{page_num+1}] cursor={cursor} 获取0条 status={result.get('status_code')}")
                ...
                print(f"  [{tag}] 分页fetch异常: {e}")

            # 尝试用aweme_type=0获取视频类型作品（移动端API默认可能只返回图文）
            try:
                for aweme_type_param in [0, 2]:
                    if len(captured_awemes) >= max_posts:
                        break
                    api_url = f'/web/api/v2/aweme/post/?reflow_source=reflow_page&sec_uid={sec_uid}&count=20&aweme_type={aweme_type_param}&max_cursor=0{extra_params}'
                    result = page.evaluate("""async (url) => {
                        try {
                            const resp = await fetch(url, {credentials: 'include'});
                            const data = await resp.json();
                    ...
                    items = result.get('items', [])
                    if items:
                        cnt = add_awemes(items, f'{tag}-type{aweme_type_param}', trust_context=True)
                        print(f"  [{tag}-type{aweme_type_param}] 获取{len(items)}条 新增{cnt}条 has_more={result.get('has_more')}")
                    elif result.get('status_code'):
                        print(f"  [{tag}-type{aweme_type_param}] 0条 status={result.get('status_code')}")
            except Exception as e:
                print(f"  [{tag}] aweme_type fetch异常: {e}")
